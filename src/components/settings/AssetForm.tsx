...
export function AssetForm({ open, asset, onClose, onSave }: Props) {
  ...
  const [hasSearched, setHasSearched] = useState(false); // Nuevo estado para controlar el error
  ...
  useEffect(() => {
    ...
    setSearching(true);
    setHasSearched(false); // Reseteamos la flag antes de buscar
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      setSearching(false);
      setHasSearched(true); // Marcamos que la búsqueda ha terminado
    }, 280);
  ...
  const handleSelect = async (m: SymbolMatch) => {
    // Limpiamos todo el UI state al momento
    setMatch(m);
    ...
    setShowDropdown(false);
    setSearching(false);
    setHasSearched(false); // Importante: apagamos la flag de error
  ...
  const livePriceEur = livePrice != null && match ? toEur(livePrice, match.currency, rates) : null;
    ...
    <>
      <style>{`
        .af-input {
          ...
          padding: 14px 16px; color: #F8FAFC; font-size: 16px; font-weight: 500;
          outline: none; transition: border-color 200ms, background 200ms;
          box-sizing: border-box;
        }
        .af-input:focus { border-color: rgba(16,185,129,0.5); background: rgba(16,185,129,0.04); }
      ...
      <div
        style={{
          position: 'fixed', inset: 0, zIndex: 9999, // Aumentado para asegurar que cubra el bottom nav
          background: 'rgba(0,0,0,0.80)', backdropFilter: 'blur(12px)',
          display: 'flex', alignItems: 'flex-end', animation: 'slideIn 0.2s both',
        }}
        onClick={onClose}
      >
        <div
          style={{
            width: '100%', maxWidth: 480, margin: '0 auto', maxHeight: '92dvh',
            background: 'linear-gradient(180deg, #141c2e 0%, #0d1520 100%)',
            border: '1px solid rgba(255,255,255,0.10)', borderRadius: '28px 28px 0 0',
        ...
        >
          ...
          <div style={{ overflowY: 'auto', flex: 1, padding: '8px 24px 32px', WebkitOverflowScrolling: 'touch' }}>
            <div style={{ marginBottom: 14, position: 'relative' }}>
              ...
              <div style={{ position: 'relative' }}>
                <input
                  ...
                  placeholder="Fidelity S&P 500, SK Hynix, MSFT…"
                  className="af-input"
                ...
                {searching && (
                  <span style={{ position: 'absolute', right: 16, top: '50%', transform: 'translateY(-50%)', fontSize: 11, color: '#64748b' }}>
                    Buscando…
                  </span>
                )}
                {match && !searching && (
                  <span style={{
                    ...
                    width: 18, height: 18, borderRadius: '50%', background: 'rgba(16,185,129,0.15)',
                ...
                {showDropdown && results.length > 0 && (
                  <div style={{
                    ...
                    background: 'rgba(20, 28, 46, 0.98)', border: '1px solid rgba(255,255,255,0.12)',
                    borderRadius: 14, padding: 6, boxShadow: '0 16px 40px rgba(0,0,0,0.55)',
                    ...
                    {results.map((r) => (
                      ...
                      >
                        <span style={{ fontFamily: 'ui-monospace, SF Mono, monospace', fontSize: 13, fontWeight: 700, color: '#10b981', minWidth: 72 }}>
                          {r.symbol}
                        </span>
                ...
                {/* Error Logic Corregida */}
                {!match && hasSearched && !searching && results.length === 0 && query.trim().length > 1 && (
                  <div style={{
                    position: 'absolute', top: 'calc(100% + 6px)', left: 0, right: 0,
                    background: 'rgba(20, 28, 46, 0.98)', border: '1px solid rgba(239,68,68,0.30)',
                    borderRadius: 14, padding: '14px 16px', fontSize: 13, color: '#fca5a5', zIndex: 5,
                  }}>
              ...
              {match && (
                <div style={{
                  marginTop: 10, background: 'rgba(16,185,129,0.07)', border: '1px solid rgba(16,185,129,0.22)',
                  borderRadius: 12, padding: '10px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8,
                }}>
                  <span style={{ fontSize: 11, color: '#94a3b8', letterSpacing: '0.06em', textTransform: 'uppercase' }}>Cotización</span>
                  <span style={{ fontSize: 14, fontWeight: 700, color: '#10b981', fontFamily: 'ui-monospace, SF Mono, monospace', textAlign: 'right' }}>
                    {priceLoading ? 'Cargando…' : livePrice && livePriceEur != null ? `${formatNative(livePrice, match.currency)} · ${formatEur(livePriceEur)}` : '—'}
                  </span>
                </div>
              )}
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 14 }}>
              <div>
                ...
                <input
                  ...
                  placeholder="0" className="af-input" step="any" min="0" disabled={!match}
                />
              </div>
              <div>
                ...
                <input
                  ...
                  placeholder="0.00" className="af-input" step="any" min="0" disabled={!match}
                />
              </div>
            </div>
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        color: isActive ? '#e2fef4' : '#64748b', fontSize: 11, fontWeight: 600, cursor: 'pointer',
                        display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, transition: 'all 0.18s',
                      }}
                    >
                      <span style={{ width: 8, height: 8, borderRadius: '50%', background: CATEGORY_COLORS[c], boxShadow: isActive ? `0 0 8px ${CATEGORY_COLORS[c]}80` : 'none' }} />
                      {CATEGORY_LABELS[c]}
                    </button>
                  );
                })}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, paddingBottom: 32 }}>
              <button
                ...
                style={{
                  ...
                  background: canSave ? 'linear-gradient(135deg, #10b981 0%, #059669 100%)' : 'rgba(255,255,255,0.05)',
                  border: 'none', borderRadius: 20, color: canSave ? '#fff' : '#475569', fontSize: 16, fontWeight: 700,
                  letterSpacing: '-0.01em', cursor: canSave ? 'pointer' : 'not-allowed', transition: 'opacity 0.18s',
              ...
              <button
                onClick={onClose}
                style={{ width: '100%', padding: '15px 0', background: 'transparent', border: '1.5px solid rgba(255,255,255,0.10)', borderRadius: 20, color: '#64748b', fontSize: 15, fontWeight: 500, cursor: 'pointer' }}
              >
                Cancelar
              </button>
            </div>
...
function formatNative(price: number, currency: string): string {
  try {
    return new Intl.NumberFormat('en-US', { style: 'currency', currency: currency === 'GBp' ? 'GBP' : currency, maximumFractionDigits: 2 }).format(currency === 'GBp' ? price / 100 : price);
  } catch {
...
function Label({ text, hint }: { text: string; hint?: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 8 }}>
      <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.07em', textTransform: 'uppercase', color: '#475569' }}>{text}</span>
